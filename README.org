...
mkdir build
cd build
cmake .. -DCMAKE_INSTALL_PREFIX=${your_choice} -Drmn_ROOT=${librmn_install_path} -DNetCDF_ROOT=${NetCDF_ROOT} [-DTEST_DATA_DIR=${test_data_dir}]
make -j 4
make install
#+end_src

If =-DTEST_DATA_DIR= is used, tests will be defined for =r.diag=.  These
